...
<openmrs:require privilege="MANAGE_CONCEPTS" otherwise="/login.htm" redirect="/module/conceptsearch/manageConceptName.form" />
...
<table class="openmrsSearchTable">
	...
	<tr>
		...
		<th><openmrs:message code="general.locale"/><a href="?sort=locale&order=desc&conceptId=${concept.conceptId}"><img style="width: 14px; height: 14px;" border="0" src="<%=request.getContextPath()%>/images/movedown.gif"></a><a href="?sort=locale&order=asc&conceptId=${concept.conceptId}"><img style="width: 14px; height: 14px;" border="0" src="<%=request.getContextPath()%>/images/moveup.gif"></a></th>
		<th><spring:message code="conceptsearch.nameLabel" /><a href="?sort=conceptName&order=desc&conceptId=${concept.conceptId}"><img style="width: 14px; height: 14px;" border="0" src="<%=request.getContextPath()%>/images/movedown.gif"></a><a href="?sort=conceptName&order=asc&conceptId=${concept.conceptId}"><img style="width: 14px; height: 14px;" border="0" src="<%=request.getContextPath()%>/images/moveup.gif"></a></th>
		<th><spring:message code="conceptsearch.tags" /></th>


	</tr>

	<c:forEach var="conceptName" items="${searchResult.pageList}" varStatus="rowStatus">
		<tr class='${rowStatus.index % 2 == 0 ? "evenRow" : "oddRow"}'>
			...
			${conceptName.conceptName}
				...
				<td>
				<c:forEach var="tag" items="${conceptName.conceptNameTags}" varStatus="rowTagStatus">
				&nbsp;
				<span class="name-tag">${tag}
			
					<span id="deleteCN${rowTagStatus.index}" class="deleteCN" >
     					<span id="deleteCN-conceptName${conceptName.conceptName}${rowTagStatus.index}" class="deleteCN-conceptName" name="${tag}" >
      					 	<a  href="#" title="Delete">x</a>
    					</span>
					<input class="hiddenAction${conceptName.conceptName}${rowTagStatus.index}" id="hiddenAction${conceptName.conceptName}${rowTagStatus.index}" type="hidden" name="hiddenAction${conceptName.conceptName}${rowTagStatus.index}" value="">
			...
			<input id="tagnameQuery${rowStatus.index}"  type="text" name="tagnameQuery${conceptName.conceptName}" size="25" value="">
			&nbsp;&nbsp;
			<input type="submit" id="saveTagWithConceptName${rowStatus.index}" class="saveTagWithConceptName" name="saveTagWithConceptName${conceptName.conceptName}" value="<spring:message code="conceptsearch.savetags" />">
			<script>
